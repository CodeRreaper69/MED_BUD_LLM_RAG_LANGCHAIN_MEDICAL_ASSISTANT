...
def configure_llm():
    ...
    if llm_opt == "llama3.2:3b":
        llm = ChatOpenAI(model_name="gpt-4o", temperature=0, streaming=False, api_key=st.secrets["OLLAMA_ENDPOINT_KEY"])
    elif llm_opt == "gpt":
        #llm = ChatOpenAI(model_name=llm_opt, temperature=0, streaming=True, api_key=os.getenv["OPENAI_API_KEY"])
        llm = ChatOpenAI(model_name="gpt-4o", temperature=0, streaming=False, api_key=st.secrets["OPENAI_API_KEY"])
    elif llm_opt == "gemini":
        llm = ChatOpenAI(model_name="gpt-4o", temperature=0, streaming=False, api_key=st.secrets["GEMINI_API_KEY"])
    else:
